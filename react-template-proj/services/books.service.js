...
function _createbook(author, maxSpeed = 250) {
  const book = getEmptybook(author, maxSpeed)
  book.id = utilService.makeId()
  return book
}

class Book {
  constructor( title, description, thumbnail, listPrice ) {
    this.id = utilService.makeId(11)
    this.title = title || utilService.makeLorem(2)
    this.description = description || utilService.makeLorem(99)
    this.thumbnail =
      thumbnail ||
    ...
    this.listPrice = {
      amount: utilService.getRandomIntInclusive(20, 120),
      currencyCode: 'EUR',
      isOnSale: false,
    }
  }
